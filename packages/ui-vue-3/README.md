...
Vue 3 wrapper component for the select/tokenfield core.

## Install

```bash
npm i @cohensive/select-vue-3 @cohensive/select-core
```
...
import { Select } from '@cohensive/select-vue-3';
...
| --- | --- | --- | --- |
| `modelValue` | `Option[]` | `[]` | Selected value(s). Use with `@update:modelValue`. |
...
```ts
import { theme } from '@cohensive/select-core';

const customTheme = {
  ...theme,
  multipleValue: (props: any) =>
    `flex m-0.5 rounded bg-gray-300 ${props.focused ? 'ring-1 ring-gray-500 ring-inset' : ''}`,
};
```

Then pass `:theme="customTheme"`.
